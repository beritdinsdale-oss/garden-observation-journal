...
function render(){
 ...
 model.sections.forEach((sec,i)=>{
  addNav(sec.section_id,sec.navigation_title);
  $('#journal-form').append(renderSection(sec,i));
 });
 addNav('summary','📈 Observation Summary');
}

function addNav(id,title){
 const li=el('li'),b=el('button','',title);
 b.type='button'; b.dataset.target=id;
 const st=el('span','status','Not started'); st.dataset.status=id;
 b.append(st); li.append(b); $('#nav-list').append(li);
 const o=el('option','',title); o.value=id; $('#section-picker').append(o);
}

/* Render items in CSV order so activity headings stay with their prompts. */
function renderSection(sec,index){
 const s=el('section','journal-section');
 s.id=sec.section_id; s.hidden=true;

 s.append(
  el('p','kicker',sec.page_label),
  el('h2','',sec.page_title),
  el('p','intro',sec.introduction)
 );

 if(sec.observation_challenge){
  const c=el('div','challenge');
  c.append(el('strong','','Observe'),el('p','',sec.observation_challenge));
  s.append(c);
 }

 if(sec.include_date_weather){
  const g=el('div','field-grid');
  g.append(
   field(sec.section_id,'date','Observation date','', 'date'),
   field(sec.section_id,'weather','Weather or conditions','Optional.', 'text')
  );
  s.append(g);
 }
 ...
 while(i<sec.items.length){
  ...
  if(item.item_type==='heading'){
   const h=el('section','activity-entry-heading');
   h.append(el('h3','',item.label));
   if(item.help_text) h.append(el('p','',item.help_text));
   s.append(h);
   i++;
   continue;
  }
  ...
  if(item.item_type==='prompt'){
   s.append(field(sec.section_id,item.item_id,item.label,item.help_text,'textarea'));
   i++;
   continue;
  }
  ...
  if(item.item_type==='checklist'){
   const group=[];
   while(i<sec.items.length && sec.items[i].item_type==='checklist'){
    group.push(sec.items[i]);
    i++;
   }
   const fs=el('fieldset','checklist');
   const lg=el('legend','', 'Notice');
   fs.append(lg);
   const g=el('div','check-grid');
   group.forEach(it=>{
    const lab=el('label','check-item'),inp=el('input');
    inp.type='checkbox';
    inp.dataset.key=`${sec.section_id}__${it.item_id}`;
    lab.append(inp,el('span','',it.label));
    g.append(lab);
   });
   fs.append(g);
   s.append(fs);
 ...
 const a=el('div','section-actions');
 const p=el('button','button','Previous section');
 const n=el('button','button primary',index===model.sections.length-1?'View summary':'Save and continue');
 p.type=n.type='button';
 p.dataset.prev=sec.section_id;
 n.dataset.next=sec.section_id;
 a.append(p,n);
 s.append(a);
 return s;
}

function field(sec,id,label,help,type){
 const w=el('div','field'),key=`${sec}__${id}`,l=el('label','',label);
 l.htmlFor=key; w.append(l);
 if(help)w.append(el('p','help',help));
 const input=type==='textarea'?el('textarea'):el('input');
 if(type!=='textarea')input.type=type;
 input.id=key; input.dataset.key=key;
 w.append(input); return w;
}

function photo(sec){
 const w=el('div','field photo'),key=`${sec}__photo`,l=el('label','','Capture an optional photo');
 l.htmlFor=key;
 const i=el('input'); i.type='file'; i.accept='image/*'; i.id=key; i.dataset.photo=key;
 ...
 w.append(l,el('p','help','The image is compressed and stored only in this browser.'),i,img,r);
 return w;
}
...
function attach(){
 ...
 $('#start').onclick=()=>show(model.sections[0].section_id,true);
 ...
 $('#journal-form').onchange=async e=>{
  ...
  try{
   ...
   state.photos[t.dataset.photo]=await compress(t.files[0]);
   showPhoto(t.dataset.photo);save();statuses();
  }catch{
   alert('The photo could not be added. Try a smaller image.');
  }
  t.value='';
 };
 ...
 $('#journal-form').onclick=e=>{
  ...
  if(n){
   const i=model.sections.findIndex(s=>s.section_id===n.dataset.next);
   show(i===model.sections.length-1?'summary':model.sections[i+1].section_id,true);
   return;
  }
  const p=e.target.closest('[data-prev]');
  if(p){
   const i=model.sections.findIndex(s=>s.section_id===p.dataset.prev);
   show(i<=0?'welcome':model.sections[i-1].section_id,true);
  }
 };

 $('#print-top').onclick=$('#print-bottom').onclick=()=>{renderSummary();window.print()};
 $('#download').onclick=download;
 $('#restore').onchange=restoreBackup;
 $('#clear').onclick=()=>{
  if(confirm('Clear all entries saved in this browser?')){
...
function show(id,hash){
 ...
 $$('.journal-section').forEach(s=>s.hidden=s.id!==id);
...
function restore(){
 $$('[data-key]').forEach(i=>{
  ...
  i.type==='checkbox'?i.checked=!!v:i.value=v||'';
  grow(i)
 });
 Object.keys(state.photos).forEach(showPhoto);
 statuses();
 setStatus(state.updatedAt?'Saved '+new Date(state.updatedAt).toLocaleString():'Saved on this device');
}

function queueSave(){
 setStatus('Saving…');clearTimeout(saveTimer);saveTimer=setTimeout(save,350)
}
...
function save(){
 ...
 try{
  ...
  setStatus('Storage full—download a backup');
  alert('Browser storage is full. Remove photos and download a backup.');
 }
}
...
function statuses(){
 model.sections.forEach(s=>{
  ...
  const st=$(`[data-status="${s.section_id}"]`);
  st.textContent=filled===0?'Not started':filled<2?'In progress':'Entry added';
 });
 $('[data-status="welcome"]').textContent='';
 $('[data-status="summary"]').textContent='Review patterns';
}
...
function renderSummary(){
 ...
 let any=false;
 model.sections.forEach(s=>{
  const chosen=s.items.filter(i=>i.item_type==='checklist'&&state.entries[`${s.section_id}__${i.item_id}`]);
  if(!chosen.length)return;
  any=true;
  const sec=el('section'),h=el('h3','',s.page_title),ul=el('ul');
  chosen.forEach(i=>ul.append(el('li','',i.label)));
  sec.append(h,ul);root.append(sec)
 });
 if(!any)root.append(el('p','help','Complete one or more Notice checklists to create this summary.'));
}

function grow(t){
 if(t.tagName==='TEXTAREA'){
  t.style.height='auto';
  t.style.height=Math.max(125,t.scrollHeight)+'px'
 }
}

function showPhoto(key){
 const img=$('#'+CSS.escape(key)+'-preview'),src=state.photos[key]||'';
 if(!img)return;
 img.src=src;img.classList.toggle('visible',!!src)
}
...
async function restoreBackup(e){
 ...
 try{
  ...
  if(!x.entries)throw 0;
  if(confirm('Replace the entries currently saved in this browser?')){
   localStorage.setItem(STORAGE_KEY,JSON.stringify(x));location.reload()
  }
 }catch{
  alert('That file is not a valid journal backup.')
 }
}
})();
